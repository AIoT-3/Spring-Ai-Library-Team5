...
강의 자료의 예시는 이미 `BookSearchService`, `BookSearchResult`, `BookSearchController`가 있다고 가정하고 시작한다. 현재 프로젝트에서는 없는 클래스를 먼저 만들어야 화면을 붙일 수 있다.

현재 프로젝트 기준 구현 순서는 아래가 좋다.

```text
1. BookRepository.search(...) 구현
2. BookSearchResult 생성
3. BookSearchService 생성
4. BookSearchController 생성
5. templates/index/index.html이 현재 패키지와 DTO 필드를 보도록 수정
```
...
| 클래스 | 책임 |
| --- | --- |
| `BookRepositoryImpl` | QueryDSL로 DB 검색, DTO projection |
| `BookSearchResult` | 검색 결과 Page를 감싸는 응답 객체 |
| `BookSearchService` | Controller와 Repository 사이의 검색 흐름 조정 |
| `BookSearchController` | 요청 파라미터를 받고 화면 모델에 결과 전달 |

성능 관점에서는 Controller나 Service에서 Entity 전체를 직접 다루지 않는 편이 좋다. 검색 목록은 필요한 컬럼만 `BookSearchResponse`로 조회하는 DTO projection을 사용한다. 이렇게 하면 목록 화면에 필요 없는 큰 컬럼이나 연관관계 로딩을 줄일 수 있다.

현재 기본 검색은 `LIKE '%keyword%'` 기반이라 구현은 단순하지만, 대량 데이터에서는 느려질 수 있다. Step 1에서는 먼저 동작하는 키워드 검색을 만들고, `04.search-optimization.md`에서 인덱스와 전문 검색을 적용하는 순서가 안전하다.
