...
현재 프로젝트의 Step 1 검색은 QueryDSL 기반의 기본 키워드 검색으로 시작한다.
...
처음부터 full-text search, vector search, hybrid search를 모두 넣으면 문제 원인을 찾기 어렵다. 그래서 성능 최적화도 단계적으로 적용한다.

```text
1단계: 기본 LIKE 검색 구현
2단계: 정확 일치/정렬에 필요한 B-Tree 인덱스 확인
3단계: 대량 텍스트 검색이 느려지면 PostgreSQL 전문 검색 적용
4단계: 의미 검색이 필요하면 vector/hybrid search로 확장
```

현재 `BookRepositoryImpl`의 기본 검색은 아래 컬럼을 대상으로 한다.
...
| 검색 조건 | 우선 방식 | 이유 |
| --- | --- | --- |
| ISBN 정확 일치 | B-Tree unique index | 가장 빠르고 안정적 |
| 제목/저자/출판사 짧은 텍스트 | LIKE 검색으로 시작 | 구현이 단순하고 화면 검증이 쉬움 |
| bookContent 긴 본문 | 전문 검색 후보 | `LIKE '%keyword%'`가 가장 느려지기 쉬움 |
| 의미 기반 추천 | Step 2 이후 vector/hybrid | 기본 검색 안정화 후 확장 |

따라서 Step 1에서는 `BookRepositoryImpl.search()`가 정상 동작하고, 페이지네이션과 count query가 맞는지 먼저 확인한다. 그 다음 병목이 확인된 컬럼에만 인덱스나 전문 검색을 적용한다.
...
PostgreSQL은 전문 검색 기능을 기본 제공한다. 현재 프로젝트에서는 먼저 `simple` 설정으로 시작하는 것이 안전하다. `simple`은 한글 형태소 분석을 하지는 않지만, 별도 확장 설치 없이 바로 쓸 수 있다.

**기본 설정으로 시작:**
```sql
-- 별도 설정 없이 simple 사용
SELECT to_tsvector('simple', '자바 프로그래밍 입문');
```

정교한 한글 형태소 분석이 필요하면 운영 DB에 맞는 확장을 검토한 뒤 별도 text search configuration을 만든다. Step 1에서는 검색 흐름과 병목 확인이 먼저다.
...
**설명:**
- `to_tsvector('simple', book_content)`: 텍스트를 토큰화하여 벡터로 변환
...
```
com.nhnacademy.ailibrarymyself.core.config.PostgreSQLFunctionContributor
```
